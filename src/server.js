...
app.post('/teams/:teamId/invite', async (req, res) => {
  try {
    const result = await inviteMember({ teamId: req.params.teamId, ...req.body });
...
app.post('/invitations/:token/respond', async (req, res) => {
  try {
    const result = await respondInvitation({ token: req.params.token, ...req.body });
...
app.get('/teams/:teamId/audit', async (req, res) => {
  try {
    ...
    const result = await getAuditLog({ teamId: req.params.teamId, startDate, endDate, limit: Number(limit) });
...
app.patch('/teams/:teamId/permissions', async (req, res) => {
  try {
    const result = await updatePermissions({ teamId: req.params.teamId, ...req.body });
...
app.post('/teams/:teamId/projects', async (req, res) => {
  try {
    const result = await createProject({ teamId: req.params.teamId, ...req.body });
