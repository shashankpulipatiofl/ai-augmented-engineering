...
function showToast(message, isError = false) {
  ...
  elements.toast.style.borderLeftColor = isError ? 'var(--danger)' : 'var(--primary)';
...
async function loadTeams() {
  try {
    ...
    if (teams.length === 0) {
      elements.teamsList.innerHTML = '<li class="nav-item" style="opacity:0.5">No teams found</li>';
    }
    ...
    teams.forEach(team => {
      ...
      li.innerHTML = `
        ...
        ${team.name}
      `;
      li.addEventListener('click', () => selectTeam(team.id, team.name, team.description));
...
async function loadProjects() {
  ...
  try {
    ...
    elements.projectsList.innerHTML = '<div class="loader"></div><p class="subtitle" style="color:var(--danger)">Failed to load</p>';
  }
...
elements.createProjectSubmit.addEventListener('click', async () => {
  ...
  if (!name) return;
  try {
    await apiCall('POST', `/teams/${currentTeamId}/projects`, { name, requesterId: currentUserId });
...
async function loadMembers() {
  ...
  try {
    ...
    elements.membersList.innerHTML = '<div class="loader"></div><p class="subtitle" style="color:var(--danger)">Failed to load</p>';
  }
...
elements.inviteSubmit.addEventListener('click', async () => {
  ...
  if (!email) return;
  try {
    await apiCall('POST', `/teams/${currentTeamId}/invite`, { email, role, requesterId: currentUserId });
...
async function loadAuditLogs() {
  ...
  try {
    ...
    let html = '<table><thead><tr><th>Action</th><th>Actor</th><th>Target</th><th>Time</th></tr></thead><tbody>';
    if (logs.length === 0) {
      html = '<p class="subtitle" style="padding:16px 24px">No recent activity.</p>';
    } else {
      logs.forEach(l => {
    ...
    elements.auditList.innerHTML = '<div class="loader"></div><p class="subtitle" style="color:var(--danger)">Failed to load</p>';
  }
